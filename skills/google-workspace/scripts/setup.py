...
# Ensure sibling modules (_garwa_home) are importable when run standalone.
_SCRIPTS_DIR = str(Path(__file__).resolve().parent)
if _SCRIPTS_DIR not in sys.path:
    sys.path.insert(0, _SCRIPTS_DIR)

from _garwa_home import display_garwa_home, get_garwa_home

GARWA_HOME = get_garwa_home()
TOKEN_PATH = GARWA_HOME / "google_token.json"
CLIENT_SECRET_PATH = GARWA_HOME / "google_client_secret.json"
PENDING_AUTH_PATH = GARWA_HOME / "google_oauth_pending.json"
...
def exchange_auth_code(code: str):
    ...
    print(f"Profile-scoped token location: {display_garwa_home()}/google_token.json")
